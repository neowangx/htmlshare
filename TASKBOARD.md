...
| V-04 | M5 | 计划限制与回收（Q3 截止点） | V-03 | done | 在 `../htmlshare-cloud` 实现计划限制与回收策略（commit `7e62525`）：free 默认 100MB 总空间、plan 驱动页数/单页体积/总空间限制、180 天未访问预警 14 天后软删、期内访问豁免、`tools/set-plan.js` 手动开通 pro；云仓 `npm test` 通过（27 tests）。 |
...
| V-06 | M5 | 部署上线（Q1 截止点） | V-04, V-05 | blocked | Q1 域名/上线路线未在 docs/00 确认；按截止点守卫冻结部署上线，不读取真实部署凭据；见 NEEDS_HUMAN H7。 |
...
- M3 增量 C-17：2026-07-16 达成。证据：真实浏览器覆盖长文任意滚动位置召回、当前章节跟随、桌面侧边抽屉、手机底部面板、跳转/遮罩/ESC/键盘焦点循环；开源仓 `npm test` 187/187 通过。
